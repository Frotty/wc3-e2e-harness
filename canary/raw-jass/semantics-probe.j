...
integer probeZero=0
string probeUpper="ABC"
string probeLower="abc"
real w=0.
integer array u
integer r=0
integer array s
...
call probeWriteFile("probe-s-control.txt","control=ok")
endfunction
function probeIntNullThread takes nothing returns nothing
if probeZero==null then
call probeWriteFile("probe-s-intnull.txt","intEqNull=TRUE_SO_SAME_AS_ZERO")
else
call probeWriteFile("probe-s-intnull.txt","intEqNull=FALSE_SO_EMITTING_ZERO_IS_WRONG")
endif
endfunction
function probeNanThread takes nothing returns nothing
local real probeR=SquareRoot(-1.)
local string probeOut=""
if probeR<0. then
set probeOut=probeOut+"lt=T "
else
set probeOut=probeOut+"lt=F "
endif
if probeR>=0. then
set probeOut=probeOut+"ge=T "
else
set probeOut=probeOut+"ge=F "
endif
if not (probeR<0.) then
set probeOut=probeOut+"notlt=T "
else
set probeOut=probeOut+"notlt=F "
endif
if probeR==probeR then
set probeOut=probeOut+"self=T"
else
set probeOut=probeOut+"self=F"
endif
call probeWriteFile("probe-s-nan.txt",probeOut)
endfunction
function probeStringCaseThread takes nothing returns nothing
local string probeOut=""
if probeUpper==probeLower then
set probeOut="eq=CASE_INSENSITIVE "
else
set probeOut="eq=CASE_SENSITIVE "
endif
if StringHash(probeUpper)==StringHash(probeLower) then
set probeOut=probeOut+"hash=CASE_INSENSITIVE"
else
set probeOut=probeOut+"hash=CASE_SENSITIVE"
endif
call probeWriteFile("probe-s-case.txt",probeOut)
endfunction
...
set Ft=null
set jt=null
call probeWriteFile("probe-s.txt","STAGE0_REACHED_MAIN")
call ExecuteFunc("probeControlThread")
call ExecuteFunc("probeIntNullThread")
call ExecuteFunc("probeNanThread")
call ExecuteFunc("probeStringCaseThread")
call probeWriteFile("probe-s.txt","STAGE_ALL_DISPATCHED")
