...
local integer probeI=41
call probeWriteFile("probe-control.txt","control="+I2S(probeI+1))
endfunction
function probeIntThread takes nothing returns nothing
local integer probeI
if probeControlFlag then
set probeI=0
endif
call probeWriteFile("probe-int.txt","uninitInt="+I2S(probeI))
endfunction
function probeIntTwiceThread takes nothing returns nothing
local integer probeI
if probeControlFlag then
set probeI=0
endif
set probeI=probeI+1
call probeWriteFile("probe-int2.txt","incremented="+I2S(probeI))
endfunction
function probeRealThread takes nothing returns nothing
local real probeR
if probeControlFlag then
set probeR=0.
endif
call probeWriteFile("probe-real.txt","uninitReal="+R2S(probeR))
endfunction
function probeBoolThread takes nothing returns nothing
local boolean probeB
if probeControlFlag then
set probeB=false
endif
if probeB then
call probeWriteFile("probe-bool.txt","uninitBool=TRUE")
else
call probeWriteFile("probe-bool.txt","uninitBool=FALSE")
endif
endfunction
function probeStringThread takes nothing returns nothing
local string probeS
if probeControlFlag then
set probeS=""
endif
if probeS==null then
call probeWriteFile("probe-string.txt","uninitString=NULL")
else
call probeWriteFile("probe-string.txt","uninitString=NOTNULL")
endif
endfunction
...
set Ft=null
set jt=null
call probeWriteFile("probe-locals.txt","STAGE0_REACHED_MAIN")
call ExecuteFunc("probeControlThread")
call ExecuteFunc("probeIntThread")
call ExecuteFunc("probeIntTwiceThread")
call ExecuteFunc("probeRealThread")
call ExecuteFunc("probeBoolThread")
call ExecuteFunc("probeStringThread")
call probeWriteFile("probe-locals.txt","STAGE_ALL_DISPATCHED")
